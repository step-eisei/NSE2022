...
def hsvbinary(img_hsv,sat_avg,val_avg):
    #画像のチャンネルを分ける
    ...
    #色相環は360度→0～255に変換
    img_h_th = np.where((im_h < 20/360*255) | (im_h > 160/360*255), 1, 0)
    img_s_th = np.where(im_s > sat_avg*0.6, 1, 0)
    img_v_th = np.where(im_v > val_avg*0.6, 1, 0)
