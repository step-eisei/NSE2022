'''
- このファイルの実行方法
 - 1. calibration_9axis.pyを実行
 - 2. カメラをコーンに向けた状態で，calibration_camera.pyを実行．
 - 3. プリントされた値を所定の位置の代入（自動化したい）
 - 4. このファイルを実行
...
def hsv_binary(img_hsv,sat_avg,val_avg):
    ...
    img_s_th = np.where(im_s > 24.541296296296295*0.9, 1, 0)
    img_v_th = np.where(im_v > 157.41085358796298*0.9, 1, 0)
